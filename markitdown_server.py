"""markitdown_server.py - MarkItDown MCP Server with stdio, SSE and Streamable HTTP."""
...
from tools.markitdown_tools import (
    ConvertToMarkdownToolHandler,
    ConvertLocalFileToolHandler,
    ConvertStreamToolHandler,
)
...
def register_all_tools() -> None:
    add_tool_handler(ConvertToMarkdownToolHandler())
    add_tool_handler(ConvertLocalFileToolHandler())
    add_tool_handler(ConvertStreamToolHandler())
...
def create_sse_starlette_app(mcp_server: Server) -> Starlette:
    ...
    async def _root_endpoint(_request) -> JSONResponse:
        return JSONResponse({"status": "ok", "transport": "sse", "server": "markitdown-mcp", "tools": len(_tool_handlers)})
...
def create_streamable_http_app(mcp_server: Server) -> Starlette:
    ...
    async def _root_endpoint(_request) -> JSONResponse:
        return JSONResponse({"status": "ok", "transport": "streamable-http", "server": "markitdown-mcp", "tools": len(_tool_handlers)})
...
async def main() -> None:
    ...
    parser = argparse.ArgumentParser(prog="markitdown-mcp", description="MarkItDown MCP Server")
    parser.add_argument("--mode", choices=["stdio", "sse", "streamable-http", "streamable_http"], default=default_mode)
    parser.add_argument("--host", default=default_host)
    parser.add_argument("--port", type=int, default=default_port)
    args = parser.parse_args()

    logger.info("MarkItDown MCP Server starting up")
